export async function toggleReaction({user, postsDB, post, reaction, reactionTotals, setReactionTotals, newValue, yourReactions, setYourReactions}) {
  const createdAt = new Date().toISOString();

  // Make the change in the database
  await postsDB.setReaction({reactorHandle: user.handle, reactingTo: post.uri, reaction, createdAt, newValue});
  ...
  setYourReactions(await postsDB.getReactionsByPerson(user.handle, post.uri));
  ...
  setReactionTotals(await postsDB.getReactionsTo(post.uri));
}
